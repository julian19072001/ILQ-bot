...
async function getXp(interaction) {
  ...
  for (const table of tables) {
    const [rows] = await db.execute(
      `
      SELECT username,
             MIN(contributed) AS startValue,
             MAX(contributed) AS endValue
      FROM \`${table}\`
  ...
  return sendPaginatedEmbed(interaction, results, formatLine, "XP contributed");
}
